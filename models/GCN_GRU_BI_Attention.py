...
class GCN_GRU_BI_Attention(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        # BiGRU expects 3D input (seq_len, batch, input_size)
        # If x is 2D, we use unsqueeze to add an extra dimension
        x = x.unsqueeze(0) if len(x.shape) == 2 else x

        x, _ = self.bigru(x)

        # x shape after GRU: (seq_len, batch, num_directions * hidden_size)
